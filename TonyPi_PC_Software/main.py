    ...
    def button_flie_operate(self, name):
        try:            
            if name == 'openActionGroup':
                ...
                # Open single .d6a file
                # parameter 1: Set parent component; 
                # Parameter 2: Title of QFileDialog
                # parameter 3: Default directory to open. '.' represents the directory where the program is running, '/' represents the root directory of the current drive
                # parameter 4: Filter for file extension of the dialog box. For example, using 'Image files (*.jpg *.gif)' will only display files with the extensions .jpg or .gif
                # set multiple file extension filters, separated by double quotation marks; “All Files(*);;PDF Files(*.pdf);;Text Files(*.txt)”
                path = openfile[0]
                try:
                    if path != '':
