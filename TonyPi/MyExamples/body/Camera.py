...
# No thread is used in this version
#
...
class Camera():
    ...
    def camera_open(self): 
        try:
            ...
            if not LAPTOP:
                self.cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc('Y', 'U', 'Y', 'V'))   # TonyPi camera Codec !
    ...
    def read(self):
        try:
            if self.opened and self.cap.isOpened(): # determine whether it is opened
                ...
                print('self.opened is False, wait for 0.01s')
                time.sleep(0.01)
        except Exception as e:
            print('Error:', e)
            time.sleep(0.01)
        finally:
            return ret, frame_tmp
...
if __name__ == '__main__':
    ...
    time.sleep(1)
    while True:
        r, img = my_camera.cap.read()
        if r and (img is not None):
